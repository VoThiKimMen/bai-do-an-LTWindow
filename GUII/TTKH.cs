...
namespace QLBH_VoThiKimMen.GUII
{
    public partial class TTKH : Form
    {
        ...
        private void btNew_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(name) || name.Length > 20 || !char.IsUpper(name[0]))
            {
            ...
            {
                // Kiểm tra xem chuỗi chỉ chứa số hay không
                if (!sdt.All(char.IsDigit))
                {
                    MessageBox.Show("Số điện thoại không hợp lệ! Vui lòng chỉ nhập số.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
            }
            else
